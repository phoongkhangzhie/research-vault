...
SECTION_STATUS: dict[str, str] = {
    ...
    # NOTE: review-board nodes (reviewer-r-L{k}, meta-review-r, revise-r) are NOT
    # in SECTION_STATUS / SECTION_KEYS — they are dynamically generated by _build_manifest
    # from review_config (N rounds, K reviewers/round, frozen at scaffold time).
    # SR-MS-REVIEW-a §5J.17.2: zero-new-DAG-mechanism, acyclic bounded unroll.
}
...
per_section_tips: dict[str, str] = {
    ...
    # ── review-board nodes (SR-MS-REVIEW-a §5J.17) ──────────────────────────
    # These nodes are dynamically generated by _build_manifest with inline specs
    # (see review_board.py's get_review_config() for N, K defaults).
    # They are NOT in SECTION_KEYS / get_active_sections() — they are always
    # added after cold-read, gated by review_config frozen at scaffold time.
    # Tip entries below are for documentation / adopter-override only.
    "reviewer": (
        "[REVIEW-BOARD reviewer node — template, see _build_manifest inline spec] "
        "You are a FRESH, INDEPENDENT adversarial reviewer. You see ONLY the compiled "
        "paper text (pdftotext output) + the review rubric. You have NOT seen any prior "
        "review or rebuttal. Score all 7 dimensions: SOUND, CONTRIB, CLARITY, ORIG, "
        "LIMIT, REPRO, ETHICS on a 1–5 venue scale. Emit machine-parseable bracket tokens: "
        "[SOUND:N] [CONTRIB:N] [CLARITY:N] [ORIG:N] [LIMIT:N] [REPRO:N] [ETHICS:N]. "
        "Justify each score in text (ARR rule). Anti-anchoring: do NOT read the manuscript "
        "note's thesis field or any prior round's review — judge from the page alone. "
        "DISCONFIRM FIRST: seek the strongest objections before any positive assessment. "
        "Node-level skip: check RunState.meta['review_board']['cleared_at'] first — "
        "if set, emit [SKIPPED] and exit (no LLM call needed). "
        "Anti-pattern: do NOT eyeball scientific merit or self-review — the multi-round "
        "adversarial board with provenance-bound floors is the gate."
    ),
    "meta-review": (
        "[META-REVIEW node — template, see _build_manifest inline spec] "
        "Aggregate K reviewer scores by MIN across reviewers (worst reviewer gates — "
        "one strong objection is never averaged away). Evaluate the floor predicate: "
        "cleared ⟺ MIN(SOUND) ≥ floor AND MIN(REPRO) ≥ floor. "
        "If cleared: write RunState.meta['review_board']['cleared_at'] = r. "
        "Synthesize the worst-three findings that survived all K reviewers. "
        "Run the canary scaffold. "
        "Write the honest meta-review: 'cleared at round r' or 'NOT CLEARED — failing: <dims>'. "
        "NEVER say 'reviewers approved' — that is the human's decision at approve-manuscript."
    ),
    "revise": (
        "[REVISE node — template, see _build_manifest inline spec] "
        "Postcondition: the revised draft STILL passes the honesty gates (anti-gaming c). "
        "(a) Record the author rebuttal to the meta-review (artifact — not a verdict; "
        "crew-cannot-self-approve: the author cannot accept their own paper). "
        "(b) Re-draft ONLY the sections identified as failing. "
        "(c) Recompile via rv manuscript compile. "
        "(d) RE-FIRE support-matcher (SR-MS-2) AND cold-read (SR-MS-COLDREAD) on the "
        "revised draft. If either BLOCKS (un-grounded or re-leaked) → reject the revision "
        "and surface the conflict: a revision CANNOT un-ground to please a reviewer. "
        "Node-level skip: if RunState.meta['review_board']['cleared_at'] set → NO-OP."
    ),
