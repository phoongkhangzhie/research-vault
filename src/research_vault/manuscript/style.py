...
per_section_tips: dict[str, str] = {
    ...
    # ── cold-read (SR-MS-AUDIENCE §5J.16.3 Layer-1 + SR-MS-COLDREAD Layer-2) ──
    # TWO-LAYER gate. Both layers are now LIVE (SR-MS-AUDIENCE + SR-MS-COLDREAD merged).
    # Runs afterok compile, parallel to critic, gates approve-manuscript.
    "cold-read": (
        "Run the TWO-LAYER cold-read gate: Layer-1 (deterministic) + Layer-2 (LLM judge). "
        "(`rv manuscript check --cold-read`, §5J.16.3). "
        "\n\n"
        "LAYER-1 — deterministic body leak-scan (no LLM, always runs):\n"
        "  Scans body sections/*.tex for internal-provenance patterns that must never "
        "appear in the public PDF (sha256 hashes, artifact paths, DAG-internal ids, "
        "sentinel text). Excludes appendix-repro.tex and data-code-availability.tex "
        "(zone-2: provenance is legitimate there). "
        "Also runs the Flag-A belt-and-suspenders scan over the pdftotext output — "
        "same patterns applied to the rendered PDF text, catching any leak the LaTeX "
        "render introduces that the .tex scan could not see. BLOCK on any hit.\n"
        "\n\n"
        "LAYER-2 — LLM cold-read judge (SR-MS-COLDREAD, requires RV_JUDGE_MODEL + ANTHROPIC_API_KEY):\n"
        "  A FRESH READER agent handed ONLY the compiled PDF text (pdftotext output) "
        "checks whether every reference, term, and cross-reference resolves from the "
        "paper's own words alone — orthogonal to whether the claims are TRUE "
        "(the support-matcher checks truth; this judge checks READABILITY for a stranger).\n"
        "\n"
        "  THREE ANTI-ANCHORING MOVES baked into the rubric (follow them as an agent):\n"
        "  (1) JUDGE FROM THE PAGE ONLY — you are FORBIDDEN to resolve any reference "
        "      using external knowledge. If a term, path, id, or cross-reference cannot "
        "      be resolved from the paper's own printed words, it DOES NOT RESOLVE — "
        "      even if you know what it means, even if it is 'obvious'. The reader you "
        "      stand in for does not have the project.\n"
        "  (2) VERBATIM SPAN OR IT DIDN'T HAPPEN — every [DANGLING] and [NEEDS-CONTEXT] "
        "      flag MUST quote the EXACT offending text, character-for-character from the "
        "      paper. If you cannot quote the literal string, you do not have a flag.\n"
        "  (3) DISCONFIRM FIRST — you MUST complete a full disconfirming sweep (Step 1 "
        "      of the rubric's procedure) BEFORE you may certify anything as self-contained. "
        "      Skipping the sweep invalidates the judgment.\n"
        "\n\n"
        "BIDIRECTIONAL CANARY — runs before trusting any real verdict:\n"
        "  (a) Known self-contained probe → MUST return [STANDS-ALONE], BLOCK_COUNT=0. "
        "      If the judge flags it → TRIGGER-HAPPY → ABORT.\n"
        "  (b) Known leaky probe → MUST return [DANGLING], BLOCK_COUNT≥2. "
        "      If the judge waves it through → BLIND → ABORT.\n"
        "\n\n"
        "VERDICTS:\n"
        "  [DANGLING]      → BLOCK (reference leads nowhere a reader can go)\n"
        "  [NEEDS-CONTEXT] → WARN (resolves but too thin; surfaced for human judgment)\n"
        "  [STANDS-ALONE]  → clean (earned only after a full disconfirming sweep)\n"
        "  UNPARSEABLE     → fail-closed BLOCK (malformed judge output cannot certify)\n"
        "\n\n"
        "Surface the output honestly: 'Layer-1: N files scanned, k BLOCK | "
        "Layer-2: 1 paper, b LLM BLOCK, w LLM WARN, f Flag-A BLOCK'. "
        "Never say 'clean' without having run both layers. "
        "A BLOCK at either layer must be resolved before approve-manuscript."
    ),
}
