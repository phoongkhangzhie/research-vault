...
_VERB_REGISTRY: dict[str, dict] = {
    ...
    "research": {
        ...
        "when_to_use": (
            ...
            "D1 (verb consolidation): 'cited-by'/'references'/'sweep' are HARD-REMOVED — "
            "forward/backward snowball and the width-sweep now run as the review-search "
            "('sweep' op) and review-snowball ('snowball-forward'/'snowball-backward' ops) "
            "TOOL nodes inside a lit-review loop's Phase-1 DAG (see 'rv review <project> new "
            "<scope> --question ...' then 'rv dag run <phase1-manifest>'). "
    ...
    "review": {
        ...
        "when_to_use": (
            "When you need to conduct a structured, pre-registered literature review "
            "using a citation-neighbor relevance walk. Use `rv review <project> new "
            "<scope> --question '...'` to scaffold the Phase-1 DAG (review-scope → "
            "[HG:approve-protocol] → review-search → review-screen → review-snowball → "
            "review-relevance-screen → review-curate → review-relevance-verify-prep → "
            "review-relevance-verify → coverage-gate (auto-resolved)) with "
            "protocol-freeze, an internal citation-neighbor relevance walk (both "
            "forward cited-by + backward references, depth-bounded by "
            "--relevance-hops, default 1), a relevance gate (mechanical off-domain "
            "pre-filter + a cold canary-verified "
            "re-check of every [NEW] paper — below the 0.30 off-domain-fraction threshold "
            ...
            "`counter-position` field — the anti-fishing structural "
            ...
            "On `coverage-gate` GO: the Phase-2 fan-out auto-emits (one `relate-<key>` "
            "node per [NEW] citekey → `review-synthesize` → `review-coverage-critic` → "
            "`approve-review` (auto-resolved)) — `rv review expand` is HARD-REMOVED (D1, "
            "verb consolidation), a no-op stub only; drive the emitted manifest with "
            ...
            "Gap-driven routing: use `rv review <project> gap-scope <gap-id> <scope>` "
            ...
            "hand-run path has no `_protocol.md` freeze, no citation-neighbor walk, and no "
            ...
            "Anti-pattern: do NOT call `rv research` stdout and scrape it for walk "
    ...
    "manuscript": {
        ...
        "when_to_use": (
            "When you need to turn `notes/` (the crew-reasoning pillar) into a "
            "submittable manuscript in `manuscripts/` (the user-facing deliverable "
            "pillar) — the notes-to-manuscript SYNTHESIS, distinct from the "
            "knowledge loops (`rv experiment`, `rv review`) that BUILD notes/ in the "
            "first place. This is the THIRD loop family, type-generic, DAG-driven "
            "like the other two. See doctrine/manuscript-loop.md for the full "
            ...
            "framework-lens-<L> xN fan-out (cold, independent lens candidates) -> "
            "framework-synthesize (select-and-graft into ONE spine) -> "
            "framework-critic (cold, rejects-only) -> approve-framework "
            "(auto-resolved, no human keypress)) — the organizing framework is a "
            "machine ensemble under a cold critic; the human's leverage is upstream "
            "(approve-protocol) and downstream (approve-manuscript). Run "
            "`rv dag run manuscripts/<slug>/phase1-dag.json`; approve-framework "
            "resolves on its own once the spine is committed (no `rv dag approve` "
            "call needed). "
            "On `approve-framework` GO, the Phase-2 draft manifest auto-emits "
            "generically from the registered type's section_set: for "
            "lit-review, N thematic sections (one per frozen framework branch) plus "
            "PRISMA scope/method, the framework/taxonomy section, cross-cutting "
            "analysis, open problems, and references -> assemble -> "
            "the 6-lens review board -> approve-manuscript (auto-resolved). "
            "`rv manuscript expand`/`rv manuscript review` are HARD-REMOVED (D1, "
            "verb consolidation), no-op stubs only; drive the emitted Phase-2 "
            "manifest with `rv dag run manuscripts/<slug>/phase2-dag.json`. "
            ...
            "(depth, width, synthesis, self-containment, adversarial, "
            "instruction-following) out over it, and `rv dag approve` ingests "
            "the verdicts for the gate decision. FLOOR axes score by "
            "MIN-across-lens (never average, never gameable); N rounds "
            "(default 2, hardcap 3, skip-once-cleared); a revise step between "
            "rounds RE-FIRES the fidelity + equation + coverage gates + the "
            "cold-agent-judged support-matcher (`rv manuscript judge-emit`/"
            "`judge-ingest`) — fail-closed (a missing/empty verdicts set halts "
            "the gate), never a config-gated skip. "
