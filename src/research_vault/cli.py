...
_VERB_REGISTRY: dict[str, dict] = {
    ...
    "manuscript": {
        ...
        "when_to_use": (
            ...
            "per-manuscript folder (manuscripts/<slug>/{_manuscript.md, main.tex, "
            "sections/, refs.bib, figures/} — NOT an OKF taxonomy, design §0/§12) "
            ...
            "Phase-1 is the framework-selection sub-loop (scope -> framework-propose "
            "-> [HG:approve-framework]) — the organizing taxonomy is a HUMAN "
            "commitment (design D5), never machine-discovered. Run "
            "`rv dag run manuscripts/<slug>/phase1-dag.json` then "
            "`rv dag approve <run_id> approve-framework` once the frozen spine has a "
            "non-empty spine_shape + branches. "
            "Use `rv manuscript <project> expand <slug>` to emit the Phase-2 draft "
            "manifest generically from the registered type's section_set: for "
            ...
            "[HG:approve-manuscript]. Sections are built from injected data (numbers, "
            ...
            "Use `rv manuscript <project> review <slug>` to run the 2-round x "
            "3-reviewer adversarial review-revise board (design §9): 3 FRESH "
            "independent reviewers per round score FLOOR axes (citation fidelity, "
            "coverage/search-reproducibility) by MIN-across-3 (never average, never "
            "gameable), plus SURFACE axes (framework soundness) and SIGNAL axes "
            "(synthesis-not-enumeration); a rebuttal/revise step between rounds "
            "RE-FIRES the fidelity + equation + coverage gates; requires "
            "RV_JUDGE_MODEL + ANTHROPIC_API_KEY — raises loudly, never silently "
            "no-ops, when no judge is configured. "
