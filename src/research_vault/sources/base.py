...
Breadth-then-depth: one narrow interface every source-adapter
implements; a normalized hit record so cross-source dedup/ranking/discounting
 never has to branch on which source produced a paper.
...
never a crash, never a silent empty list masquerading as a real zero (
...
class PaperHit:
    ...
    consume) — this is the zero-behavior-change seam: the S2
    path was refactored to *produce* PaperHit, but existing dict-consuming
    code keeps reading ``hit.raw`` unchanged.

    ``source`` is the adapter's ``name`` (e.g. ``"semantic-scholar"``,
    ``"arxiv"``, ``"openalex"``, ``"pubmed"``) — the provenance the dedup
    unions and utility ranker (source-diversity dim) count.
    """
