...
_DEFAULT_REVIEW_TIPS: dict[str, str] = {
    "review_scope_tips": (
        ...
        "missing.\n"
        "      B. Expand EACH facet into ~3-6 terms: canonical phrasing + "
    ...
    "per_paper_relate_tips": (
        ...
        "standard markdown links, explicitly not wikilinks):\n"
        "      `- [SUPPORTS|CONTRADICTS|PARTIAL|EXTENDS] [<display text>]"
        "(/literature/<citekey>.md) — <why, in your own words>`\n"
        "    e.g. `- [SUPPORTS] [Baltaji 2024](/literature/"
        "baltajipersonainconstancymulti2024.md) — replicates the same "
        "persona-drift mechanism in a related population.`\n"
        ...
        "the bracket TAG derives the relation kind mechanically.) The "
        "trailing `(reciprocal|refutational|line-of-argument)` mirror is "
        "OPTIONAL — you may add it for clarity, but the TAG is authoritative: "
        "if you add it and it disagrees with the tag, the tag wins and the "
        "disagreement is surfaced, never silently resolved. Do NOT omit the "
        "tag+link to save a word — a bracket-tag-shaped line that does not "
        "fully parse (a typo'd tag, a bare citekey with no markdown link, a "
        "missing target) is a hard FAIL, surfaced loudly, never silently "
        "dropped — this is checked via a FULL-BODY scan, so even a line "
        "placed under the wrong heading is still caught, but the canonical "
        "`## Related papers` heading is REQUIRED regardless (downstream "
        "traversal depends on it). The typed TAG + TARGET are required and "
        ...
        "tag you may use at that retrieval tier is `[PARTIAL]`. This is "
        ...
        "  - Format: `[SUPPORTS] [<display text>](/concepts/<slug>.md) — "
        "<one sentence why>`\n"
        "  - Format: `[CONTRADICTS] [<display text>](/concepts/<slug>.md) — "
        "<one sentence why>`\n"
        "  - Format: `[PARTIAL] [<display text>](/concepts/<slug>.md) — "
        "<one sentence why>`\n"
        "  - e.g. `[SUPPORTS] [WEIRD default](/concepts/western-consensus-"
    ...
    "review_synthesize_tips": (
        ...
        "`[SUPPORTS|CONTRADICTS|PARTIAL|EXTENDS] <citekey> — <reason>` lines "
        "already say so mechanically; scan for those lines across the corpus "
