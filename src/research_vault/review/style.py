...
_DEFAULT_REVIEW_TIPS: dict[str, str] = {
    ...
    "review_search_tips": (
        "Execute the PARALLEL WIDTH-SWEEP using the frozen protocol from "
        "`_protocol.md` — do not modify the inclusion/exclusion criteria, the "
        "angle matrix, or the sources list seen in the protocol.\n\n"
        ...
        "assembled. The audit trail IS the deliverable, not a by-product. HR's "
        "width-sweep craft (breadth-then-depth, NG-1..3): fetch broad across "
        "MULTIPLE sources and MULTIPLE analytical angles before the depth "
        "snowball narrows — the Palatsi miss (8 near-synonym seeds missed a "
        "paper only the depth snowball recovered) is the failure this fixes.\n\n"
        "Search discipline:\n"
        "  - Run `rv research sweep <path/to/_protocol.md> --project <slug>` — "
        "this executes the cross-product (angle x source-adapter) CONCURRENTLY "
        "under the ~65-source fetch budget (`--budget`, D4's validated "
        "diminishing-returns range), cross-source dedups (DOI > arXiv > OpenAlex "
        "> normalized-title, union of external ids), discounts `derivative-of` "
        "near-duplicate restatements (NG-9 — annotated `[DERIVATIVE-OF:...]`, "
        "never deleted), and ranks+selects via the 6-dim utility score "
        "(Authority/Novelty/Stance-diversity/Coverage/Redundancy/Freshness) "
        "with a ≥3-independent-source floor that NEVER caps a boundary item — "
        "a `[BELOW-FLOOR: needs more sources]` item is a signal for the depth "
        "snowball to keep chasing it, not a paper to drop.\n"
        "  - Annotate every result: `[NEW]` (not in corpus) or `[IN-CORPUS:<citekey>]` "
        "(already filed) — the sweep does this automatically against the real "
        "corpus index.\n"
        "  - Apply inclusion/exclusion from the protocol. Record each excluded paper "
        "with the criterion that excluded it (audit trail).\n"
        "  - Record all hits (not just accepted papers, and not just the kept-after-"
        "budget set) so saturation is measurable and the residue is honest.\n\n"
        "Output: a `_search_hits.md` log of the sweep — every (angle, source) cell's "
        "result count, any degraded/errored cells (an adapter down degrades that "
        "cell gracefully, never fails the whole sweep), and every annotation."
    ),
    "review_snowball_tips": (
        ...
        "  6. `derivative-of` discounting (NG-9, HR-craft rec 3): flag — do NOT "
        "delete — any `[NEW]` candidate that is a >60%-overlap near-duplicate "
        "restatement of a paper already in `_corpus.md` (a preprint vs its "
        "camera-ready twin, or a citation-ancestry restatement). `derivative.py`'s "
        "`mark_derivatives`/`count_independent` (research_vault.sources) give the "
        "mechanical check; ANNOTATE it `[DERIVATIVE-OF:<citekey>]` in `_corpus.md`, "
        "keep it in the corpus (provenance preserved), but the STOP rule below "
        "reads the INDEPENDENT (non-derivative) count, never the raw count — an "
        "exploding intersection of near-duplicate restatements must not look like "
        "fresh evidence and mask a real plateau.\n\n"
        "STOP (PRIMARY rule, unchanged) when 2 CONSECUTIVE rounds yield:\n"
        "  - 0 new INDEPENDENT `[NEW]` citekeys (forward + backward combined, "
        "`derivative-of`-flagged restatements excluded from this count), AND\n"
        "  - 0 new concept-tags.\n"
    ...
    "per_paper_relate_tips": (
        ...
        "A paper is more than prose (§7.5 — taken LEAN: record what you see, "
        "never fetch or download it). While you read the paper, extract three "
        "more things in the SAME pass:\n\n"
