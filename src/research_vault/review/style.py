...
_DEFAULT_REVIEW_TIPS: dict[str, str] = {
    ...
    "per_paper_relate_tips": (
        ...
        "A paper is more than prose (§7.5 — the AutoSOTA `AgentResource` insight, "
        "taken LEAN: record what you see, never fetch or download it). While you "
        "read the paper, extract three more things in the SAME pass:\n\n"
        ...
        "ledger keyed by the SAME label used in the body:\n"
        "           key_equations:\n"
        "             - label: eq:elbo\n"
        "               critical: true\n"
        "             - label: eq:kl\n"
        ...
        "`critical: false` for a supporting or incidental equation. A paper with "
        "no pivotal equations gets NO `## Key equations` section and NO "
        "`key_equations:` field at all — never a placeholder or an empty "
        "guess. This ledger is what the downstream manuscript loop's "
        "equation-fidelity gate reads; a dropped `critical: true` equation is a "
        "BLOCK there, so mark conservatively and only from what the paper "
        "actually states.\n"
        "  2. `repo:` — the paper's code repository URL, if it discloses one "
        "(front-matter footnote, abstract, a project page linked from the "
        "paper). Leave `repo:` empty when the paper ships no code — do not "
        "guess or search beyond what you already read.\n"
