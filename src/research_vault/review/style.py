...
_DEFAULT_REVIEW_TIPS: dict[str, str] = {
    ...
    "review_screen_tips": (
        ...
        "snowball walk to keep chasing it, not a paper to drop outright.\n"
        "  - Accept the surviving `[NEW]` papers as the seed frontier for the "
        "snowball tool node.\n\n"
        "Output: `_screen.md` — one line per ACCEPTED seed paper's best "
        "identifier (DOI/arXiv id/S2 id, one per line, no other prose on "
        "those lines — the `review-snowball` tool op parses this file "
        "directly), preceded by a prose audit trail of every exclusion and "
        "why."
    ),
    "review_curate_tips": (
