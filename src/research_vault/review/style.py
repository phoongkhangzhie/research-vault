...
_DEFAULT_REVIEW_TIPS: dict[str, str] = {
    ...
    "per_paper_relate_tips": (
        ...
        "relation at all (do not over-rigidify this). These typed edges are "
        "carried structure, not decoration: `review-synthesize` reads them "
        "directly from each `literature/<key>.md` note's `## Related papers` "
        "section (its `reads:` already include the `literature/` directory), "
        "and at manuscript-compile time `review.relations_report()` renders "
        "the same edges mechanically into the comparative-relations section — "
        "a real edge here is never re-derived from prose downstream. Catch "
        "near-tautologies: if a "
