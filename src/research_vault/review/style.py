...
_DEFAULT_REVIEW_TIPS: dict[str, str] = {
    ...
    "per_paper_relate_tips": (
        ...
        "in your own words> (reciprocal|refutational|line-of-argument)`\n"
        "    ([SUPPORTS]≈reciprocal, [CONTRADICTS]≈refutational, "
        "[PARTIAL]/[EXTENDS]≈line-of-argument.) The typed TAG + TARGET are "
        "required and checked; the RELATION'S SUBSTANCE (why/where it holds) "
        "stays free prose — a relation reduced to a bare tag with no reasoning "
        "is as thin as no relation at all (do not over-rigidify this). These "
        "are traversed downstream by `rv review <project> relations <scope>` — "
        ...
        "`## Related papers` section is needed.\n\n"
    ...
    "review_synthesize_tips": (
        ...
        "command already surfaces every typed edge in the corpus.\n\n"
        "Outputs:\n"
